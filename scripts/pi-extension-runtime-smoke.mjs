...
async function asyncSubagentContractExpectedRed(evidence) {
  ...
  try { await waitForSmokeCondition(() => callbackForStatus("failed", failedCallbackStart), { label: "failed callback shape", timeoutMs: 10_000 }); } catch {}
  ...
  try { siblingARunning = await waitForSmokeCondition(() => mod.subagentPresentationLogForTests().find((entry) => entry.call_id === "cancel-source-a" && entry.status === "running"), { label: "sibling A running", timeoutMs: 10_000 }); } catch {}
  try { siblingBRunning = await waitForSmokeCondition(() => mod.subagentPresentationLogForTests().find((entry) => entry.call_id === "cancel-source-b" && entry.status === "running"), { label: "sibling B running", timeoutMs: 10_000 }); } catch {}
  ...
  try { await waitForSmokeCondition(() => callbackForStatus("cancelled", userCancelCallbackStart), { label: "cancelled callback shape", timeoutMs: 10_000 }); } catch {}
...
`, "utf8");
    ...
    await copyFile(join(dirname(extensionPath), "session-timeline-worker.mjs"), join(tempRoot, "session-timeline-worker.mjs"));
