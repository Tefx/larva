...
from larva import cli_entrypoint, cli_facade
...
from larva.shell import cli, cli_runtime
...
class TestRunCli:
    ...
    def test_assemble_override_parse_error_returns_critical(self) -> None:
        ...
        assert exit_code == EXIT_CRITICAL
        assert stdout.getvalue() == ""
        assert "invalid choice" in stderr.getvalue() or "unsupported command" in stderr.getvalue()
    ...
    def test_component_list_json_routes_to_component_store_only(self) -> None:
        ...
        assert exit_code == EXIT_CRITICAL
        assert json.loads(stdout.getvalue())["error"]["code"] == "INTERNAL"
...
class TestDispatch:
    ...
    def test_assemble_with_bad_override_returns_critical(self) -> None:
        ...
        failure = result.failure()
        assert failure["exit_code"] == EXIT_CRITICAL
        assert "unsupported command" in failure["error"]["message"].lower()
    ...
    def test_component_list_routes_to_component_list_handler(self) -> None:
        ...
        assert isinstance(result, Failure)
        assert result.failure()["exit_code"] == EXIT_CRITICAL
        # Removed component commands should NOT call facade methods
        assert facade.validate_calls == 0
        assert facade.assemble_calls == 0
        assert facade.register_calls == 0
        assert facade.resolve_calls == 0
...
class TestCLIAssemblyComponentCommandsRemoved:
    ...
    def test_assemble_command_does_not_exist(self) -> None:
        ...
        assert "assemble" not in subparsers_action.choices, (
            "CLI 'assemble' command still exists. "
    ...
    def test_component_command_does_not_exist(self) -> None:
        ...
        assert "component" not in subparsers_action.choices, (
            "CLI 'component' command still exists. "
