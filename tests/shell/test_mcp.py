...
class TestMCPToolDefinitions:
    ...
    def test_component_list_tool_is_defined(self) -> None:
        """Verify larva_component_list is absent after cutover."""
        tool_names = [t["name"] for t in mcp_module.LARVA_MCP_TOOLS]
        assert "larva_component_list" not in tool_names

    def test_component_show_tool_is_defined(self) -> None:
        """Verify larva_component_show is absent after cutover."""
        tool_names = [t["name"] for t in mcp_module.LARVA_MCP_TOOLS]
        assert "larva_component_show" not in tool_names
...
class TestMCPErrorCodes:
    ...
    def test_component_error_codes_are_removed(self) -> None:
        ...
        """
        # Codes remain for non-public/internal component storage paths while public
        # MCP component tools are removed.
        assert "larva_component_list" not in [t["name"] for t in mcp_module.LARVA_MCP_TOOLS]
...
class TestMCPMalformedParamsRejected:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("larva_variant_list", {"id": 3}, "parameter 'id' must be string"),
            (
                "larva_resolve",
                {"id": "ok", "overrides": []},
    ...
    def test_all_tools_reject_malformed_payloads(
        ...
        dispatch = {
            ...
            "larva_variant_list": handlers.handle_variant_list,
    ...
    @pytest.mark.parametrize(
        "tool",
        ["larva_validate", "larva_variant_list", "larva_resolve", "larva_register", "larva_list"],
    )
