...
class TestThinAdapterSemantics:
    ...
    def test_public_api_limited_to_documented_functions(self) -> None:
        ...
        # validate, register, resolve, list, and variant operations; removed
        # assembly/component helpers must not remain public API.
        expected_functions = {"validate", "register", "resolve", "list", "variant_list", "variant_activate", "variant_delete"}
    ...
    def test_wrapper_functions_callable(self) -> None:
        ...
        assert callable(getattr(python_api, "variant_list", None))
        assert callable(getattr(python_api, "variant_activate", None))
        assert callable(getattr(python_api, "variant_delete", None))
...
class TestPythonApiExports:
    ...
    def test_assemble_in_all(self) -> None:
        """assemble must not be in __all__ after cutover."""
        assert "assemble" not in python_api.__all__
    ...
    def test_component_list_in_all(self) -> None:
        """component_list must not be in __all__ after cutover."""
        assert "component_list" not in python_api.__all__

    def test_component_show_in_all(self) -> None:
        """component_show must not be in __all__ after cutover."""
        assert "component_show" not in python_api.__all__

    def test_variant_exports_in_all(self) -> None:
        """variant operations must be in __all__ after cutover."""
        assert "variant_list" in python_api.__all__
        assert "variant_activate" in python_api.__all__
        assert "variant_delete" in python_api.__all__
