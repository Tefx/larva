    ...
    payload = _run_node_inline(
        ...
        textwrap.dedent(
            ...
            Object.assign(hostSession, {{
              ...
              _emit: () => undefined,
              _runAgentPrompt: async function(messages) {{ await this.agent.prompt(messages); }},
            }});
            const piRuntimeCore = {{
              setModel: () => true,
...
def test_installed_pi_model_map_profile_switch_uses_real_runtime_and_child_rpc() -> None:
    ...
    assert payload["package"]["versionText"] == "0.84.4"
    assert payload["package"]["installedVersion"] == "0.84.4"
    assert payload["rpc"]["attempted"] is True
    assert payload["rpc"]["supported"] is True
...
def test_installed_child_pi_model_map_profile_switch_emits_raw_real_process_evidence() -> None:
    ...
    assert all(child["package_version"] == "0.84.4" for child in children)
