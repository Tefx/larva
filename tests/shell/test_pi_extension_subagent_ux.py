...
def test_larva_subagent_background_indicator_count_only_expected_red(tmp_path: Path) -> None:
    ...
    payload = _run_node(
        ...
        console.log(JSON.stringify({
          ...
          statusTexts,
          backgroundTexts,
          activeCountOnly: backgroundTexts.some((text) => /^subagents: \\d+ running$/.test(text) || /^🤖 \\d+/.test(text)),
          labelOptimized: backgroundTexts.every((text) => (text.startsWith("subagents:") || text.startsWith("🤖")) && !text.includes("bg") && !text.startsWith("Larva:")),
...
def test_subagent_result_presentation_console_and_callback_renderer(tmp_path: Path) -> None:
    ...
    assert payload["longMarkdownCompactLineCount"] <= 19
...
def test_vt46_render_result_final_views_parent_footer_and_no_dashboard(tmp_path: Path) -> None:
    ...
    payload = _run_node(
        ...
        console.log(JSON.stringify({
          ...
          parentFooterPreserved: statuses.some(([key, value]) => key === "larva" && (value === "🎭 ok" || value === "larva: ok")),
