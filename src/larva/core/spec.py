...
class PersonaSpec(TypedDict, total=False):
    ...
    This TypedDict defines the complete shape of a persona specification under
    the ADR-002 transition where capability intent is represented by
    ``capabilities``.
    ...
    Fields:
        ...
        capabilities: Canonical mapping of capability names to posture
            classifications.
        tools: DEPRECATED alias for ``capabilities`` kept for transition
            compatibility.
        model_params: Additional model parameters (temperature, top_p, etc.).
        side_effect_policy: DEPRECATED policy governing side-effectful
            operations.
